...
export default function BrowseTripsClient({
    ...
    const [activeCollection] = useState(initialCollection);
    ...
    const filteredTrips = tripsData.filter(trip => {
        if (activeCollection !== 'all' && !trip.tags.includes(activeCollection)) return false;
    ...
    const collectionTitles: Record<string, { title: string; crumb: string; intro: string }> = {
        all: {
            title: 'Browse Trips',
            crumb: 'Browse Trips',
            intro: 'Explore our full collection of tailor-made Bhutan journeys.'
        },
        offers: {
            title: 'Special Offers',
            crumb: 'Special Offers',
            intro: 'Current savings and limited-time departures curated for flexible travelers.'
        },
        new: {
            title: 'New Trips',
            crumb: 'New Trips',
            intro: 'Fresh itineraries shaped around new routes, stays, and seasonal experiences.'
        },
        popular: {
            title: 'Most Popular',
            crumb: 'Most Popular',
            intro: 'The journeys travelers return to again and again.'
        }
    };
    const pageContent = collectionTitles[activeCollection] ?? collectionTitles.all;
    const hasPresetFilter = Boolean(initialType || initialActivity);
    const presetTitle = initialType ?? initialActivity ?? '';
    const headerTitle = hasPresetFilter ? presetTitle : pageContent.title;
    const headerIntro = initialType
        ? `Curated journeys matched to the ${initialType} travel style.`
        : initialActivity
            ? `Trips matched to ${initialActivity.toLowerCase()} experiences.`
            : pageContent.intro;
    const breadcrumbLabel = hasPresetFilter ? presetTitle : pageContent.crumb;
                ...
                <div className="container browse-grid">
                    <aside className={`filters-sidebar ${isFilterDrawerOpen && isMounted ? 'is-open' : ''}`}>
                        ...
                        <div className="filter-inner">
                            ...
                            <div className="filter-group">
                                ...
                                <div className="filter-options">
                                    {['Cultural Immersion', 'Trekking & Hiking', 'Wildlife Safaris', 'Photography', 'Nature & Wellness'].map(act => (
                                        <label key={act}>
                                            <input
                                                type="checkbox"
                    ...
                    <div className="results-area">
                        <div className="results-header">
                            <div>
                                <h2>{headerTitle}</h2>
                                <p style={{ margin: '8px 0 0', color: '#666', fontSize: '0.95rem' }}>{headerIntro}</p>
                            </div>
                        ...
                        <div style={{ marginBottom: '20px', color: '#666', fontSize: '0.95rem' }}>
                            Showing {filteredTrips.length} result{filteredTrips.length === 1 ? '' : 's'}
                        </div>
